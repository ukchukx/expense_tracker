defmodule ExpenseTracker.TelemetryReporter do
  ...
  def handle_event([:commanded, :command, :dispatch, :start], _measurements, %{command: command} = _metadata, nil) do
    Logger.debug("Command #{inspect(command.__struct__)} dispatched")
  end

  def handle_event([:commanded, :command, :dispatch, :success],
  ...
  def handle_event([:commanded, :command, :dispatch, :failure], _measurements, %{command: command} = _metadata, nil) do
    Logger.debug("Command #{inspect(command.__struct__)} failed")
  end

  def handle_event([:commanded, :event, :published],
